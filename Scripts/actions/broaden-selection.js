...
function expandRangeToWordBoundaries(range, editor) {
	const bounds = new Range(0, editor.document.length)
	let head = range.start
	let tail = range.end
	// Parse backward until we find a non-word character
	while (head > 0) {
		head -= 1
		const char = editor.getTextInRange(new Range(head, head + 1))
		// The isWordChar regex is defined at the end of the file, because it's gnarly
		if (!isWordChar.test(char)) {
			// Bump back up to the previous index
			head += 1
			break
		}
	}
	// Parse forward until we find a non-word character
...
nova.commands.register('handy.broadenSelection', editor => {
	...
	for (const range of editor.selectedRanges) {
		...
		// If the ranges match, try to grab the nearest word, then expand to the current line contents
		if (delimitedRange.isEqual(range)) {
			delimitedRange = expandRangeToWordBoundaries(range, editor)
			// If we don't match a word boundary, then expand to the current line contents and call it good
			if (delimitedRange.isEqual(range)) {
				const lineRange = editor.getLineRangeForRange(range)
				delimitedRange = getTrimmedRange(lineRange, editor)
				// And finally make sure that we didn't shrink the range
				if (range.containsRange(delimitedRange) && !range.isEqual(delimitedRange)) {
					delimitedRange = range
				}
			}
		}
		// TODO: any other automatic expansions? Maybe select everything at the same indent level if the line is selected?
		newSelectedRanges.push(delimitedRange)
	}
	editor.selectedRanges = newSelectedRanges
})

// Unicode letter regex thanks to XRegExp: http://xregexp.com/
